...
export const loginUser = async(req,res) =>{
    ...
    try {
        const userExits = await User.findOne({clerkId})
        if(userExits){
            return res.json({success : true});
        }

        const user = await User.create({ clerkId, email, firstName, lastName, profileImage })
...
// export const oginUser = async(req,res) =>{
